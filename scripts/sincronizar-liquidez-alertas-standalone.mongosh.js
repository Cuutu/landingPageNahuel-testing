/**
 * Script STANDALONE para sincronizar liquidityData de alertas con distribuciones de liquidez
 * 
 * OBJETIVO:
 ...
 * 
 * O ejecutar con: mongosh <connection_string> < scripts/sincronizar-liquidez-alertas-standalone.mongosh.js
 */
...
["TraderCall", "SmartMoney"].forEach(function(pool) {
  ...
  activeDistributions.forEach(function(dist, index) {
    ...
    if (sharesDiff < 0.0001 && allocatedDiff < 0.01) {
      ...
      // Si la distribución tiene soldShares, significa que se vendió parcialmente
      // Los originales deberían ser: shares actuales + soldShares
      const soldShares = toNumber(dist.soldShares) || 0;
      const originalShares = soldShares > 0 
        ? distributionShares + soldShares 
        : (toNumber(alert.liquidityData?.originalShares) || distributionShares);
      const originalAllocated = originalShares * entryPrice;
      ...
      if (updateResult.modifiedCount > 0) {
        ...
        if (soldShares > 0) {
          print(`         Original Shares: ${formatNum(originalShares, 4)} (actual: ${formatNum(distributionShares, 4)} + vendido: ${formatNum(soldShares, 4)})`);
        }
        totalUpdated++;
      } else {
  ...
  if (inactiveDistributions.length > 0) {
    ...
    inactiveDistributions.forEach(function(dist, index) {
      ...
      if (alert) {
        ...
        // Calcular valores originales (shares vendidos * entryPrice)
        const originalShares = soldShares;
        const originalAllocated = originalShares * entryPrice;
        ...
        if (currentShares > 0.0001 || currentAllocated > 0.01) {
          ...
          // Asegurar que originalShares y originalAllocated estén correctos
          const currentOriginalShares = toNumber(alert.liquidityData?.originalShares) || 0;
          const currentOriginalAllocated = toNumber(alert.liquidityData?.originalAllocatedAmount) || 0;
          
          if (Math.abs(currentOriginalShares - originalShares) > 0.0001 || 
              Math.abs(currentOriginalAllocated - originalAllocated) > 0.01) {
            print(`         ⚠️  Actualizando valores originales:`);
            print(`            Original Shares: ${formatNum(currentOriginalShares, 4)} → ${formatNum(originalShares, 4)}`);
            print(`            Original Allocated: $${formatNum(currentOriginalAllocated)} → $${formatNum(originalAllocated)}`);
            ...
            totalUpdated++;
            print(`         ✅ Valores originales actualizados`);
          } else {
            print(`         ✅ Ya está sincronizado (shares = 0, valores originales correctos)`);
          }
        }
      } else {
        print(`         ⚠️  Alerta no encontrada (AlertId: ${alertId})`);
...
print("   - Se actualizaron originalShares y originalAllocatedAmount basándose en soldShares");
print("   - Se procesaron tanto distribuciones activas como inactivas (vendidas completamente)");
print("");
