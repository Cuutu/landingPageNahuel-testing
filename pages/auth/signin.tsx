...
/**
 * Página de inicio de sesión usando formulario HTML nativo
 * Más confiable que JavaScript signIn() en navegadores con tracking prevention
 */
export default function SignInPage({ callbackUrl, error, csrfToken: initialCsrfToken }: SignInProps) {
  ...
  const handleSubmit = () => {
    if (!csrfToken) {
      // Si no hay token, obtenerlo y reintentar
      getCsrfToken().then((token) => {
        if (token) {
          setCsrfToken(token);
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ 
          ...
          {/* Formulario HTML nativo - más confiable que JavaScript */}
          <form 
            ref={formRef}
            method="post" 
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    // Pre-obtener CSRF token del servidor para el formulario
    // Esto acelera el primer clic
    let csrfToken = '';
    try {
      ...
      // Si falla, el cliente lo obtendrá
    }
